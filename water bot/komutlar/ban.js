...
exports.run = async (bot, message, args) => {
    ...
    if (!args[0]) return message.channel.send(":x: Yasaklamak istediğiniz kullanıcıyı etiketleyiniz.")
    ...
    message.channel.send(`${user.tag}, adlı kullanıcıyı sunucudan yasaklayacağım emin misiniz? Eminseniz \`e\` işlemi iptal etmek ise \`h\` olarak cevaplayınız.`)
        let uwu = false;
            while (!uwu) {
                ...
                message.channel.send(`<a:aa:741713608215494697> **${user.tag}** adlı kullanıcı sunucudan yasaklandı.`)
